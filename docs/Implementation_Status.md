...
| **글자색 · 배경색** (인라인 `<span style>`)              |  ✅  | `editor-core/colorTools.jsx`   |
| **저장 전 좌우 비교** (내 편집 ↔ 파일 원본, 줄 맞춤)         |  ✅  | `md-editor/diffRows.js`        |
| **멎음·느린 호출 기록** (숨김 폴더 `.mdlog`)              |  ✅  | `md-core/diag.rs` · `editor-core/diag.js` |
...
| 트리 층 맞춤 · 글씨 크게                   |  ✅  | 파일이 폴더보다 오른쪽, 13.5px                    |
...
| 큰 문서 타이핑 비용(위지윅/원본 · 영문/한글 조합) · 한글 조합이 다섯 자리에서 보이는가 | 실측(CDP) | 1.02초 → 0.59초 |
| 색 단추·색판·마크다운 왕복 · 트리 층 · 좌우 비교 | 헤들리스 브라우저 10건 | 통과 |
