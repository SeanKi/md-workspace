...
나와 층이 어긋나 보였다. 파일에도 화살표 자리(14px)를 그대로 주어 **아이콘이 형제 폴더와
정확히 같은 자리**에 서게 했다. 트리 글씨는 12px → 13.5px.

처음에는 6px 더 밀어 파일을 오른쪽에 두었는데, 눈으로 보니 그만큼이 오히려 층을
어긋나 보이게 했다. 층은 들여쓰기 한 칸(16px)이 말하고, 폴더인지 파일인지는 아이콘이
말한다 — 둘을 겹쳐 말할 이유가 없다.

접고 펴는 화살표도 작고 흐려서 안 보였다. 10px → 15px(1.5배), 색도 `#98a0aa` →
`#6b7280` 으로 진하게. `line-height: 1` 로 묶어 글자를 키워도 줄 높이(26px)는 그대로다.
화살표 칸 폭은 `TreeNode.jsx` 의 `CARET` 과 짝이라 함께 고쳐야 층이 안 어긋난다.
...
### 3. "MD Notepad 로 열기" 가 실행 파일을 못 찾던 문제

실제로 걸렸다. **한 자리만 봤기 때문이다** — "이 앱 옆". portable 을 여기저기 두는
것이 이 앱의 쓰임새인데(실제로 배포 자리가 둘이었다) 그 전제가 틀렸다.

이제 세 곳을 차례로 본다.

| 순서 | 어디 | 왜 |
|---|---|---|
| 1 | 설정에 적어 둔 경로 | 한 번 골라 주면 그다음부터 그대로 |
| 2 | 이 앱 옆 | `deploy.bat` 이 둘을 같은 폴더에 넣는다 |
| 3 | **`.md` 를 여는 것으로 등록된 프로그램** | 레지스트리에 경로가 통째로 있다. 다른 폴더에 두었어도 찾힌다 |

그리고 **메뉴 항목을 늘 보이게 했다.** 예전에는 못 찾으면 항목을 아예 감췄는데,
그러면 "왜 없지?" 로 끝나고 사용자가 할 수 있는 일이 없다. 지금은 눌렀을 때 못 찾으면
**직접 고르게 하고 그 경로를 `MDSyncNote.ini` 에 적어 둔다** — 다음부터는 바로 열린다.
설정 줄에도 "지금 어느 것을 쓰는지" 를 보여 주고 [찾아보기] · [자동으로] 를 뒀다.

실패 메시지도 **어디를 봤는지** 말한다. "찾을 수 없습니다" 만으로는 다음에 무엇을
해야 할지 알 수 없다.
...
| 실행 파일 찾기 — 설정 우선 · 없는 경로는 건너뜀 · 옆에서 찾으면 같은 폴더 · 레지스트리 결과는 실재 | `cargo test` 5건 | 통과 |
| 파일 메뉴에 열기가 늘 있다 · 폴더에는 없다 · 설정에 경로 줄 | 헤들리스 브라우저 5건 | 통과 |
...
## 2026-09-09 — v0.12.1 · 한글 조합이 늦는 것 — 재현 대신 앱이 말하게 했다

"한글을 쓸 때 글씨가 바로 안 보이는 경우가 있다. 되다 안 되다 한다."

### 짐작을 두 번 틀렸다

1. v0.11.0 에서 "멈춤 때문" 이라 보고 타이핑 비용을 절반으로 줄였다 → 여전히 났다
2. `@lexical/markdown` 의 단축키 변환을 의심했다 → 소스에 `isComposing()` 가드가 있었다.
   원인이 아니다

### 재서 알아낸 것

**조합 한 단계에 250~600ms 가 든다** (실사용 문서 268KB 기준). 그 안에서

| 무엇 | 얼마 |
|---|---|
| MDXEditor 가 문서 전체를 마크다운으로 다시 쓰는 것 | 50ms |
| **Lexical 이 큰 문서 트리를 다시 맞추는 것** | **~250ms** |

update listener 26개를 조합 중에 통째로 떼어내고 재 봤더니 293ms → 243ms 였다.
즉 **남은 비용의 대부분은 MDXEditor 가 아니라 Lexical 자체**다. 우리가 줄일 수 없다.

한 글자에 0.25~0.6초가 들면 조합 중인 글자가 제때 그려지지 못한다. 그리고 그 시간은
문서 크기·그 순간의 부하에 따라 달라지므로 **"되다 안 되다" 로 보인다.**

Lexical 안에도 부하에 따라 길이 갈리는 곳이 있다 — `$handleCompositionStart` 는
**키 누름과 조합 시작 사이가 30ms 를 넘느냐**로 다른 처리를 한다. 앱이 바쁘면 이 값이
커진다. 이것이 증상에 관여하는지는 확인하지 못했다.

### 그래서 — 앱이 스스로 적게 했다 (`imeWatch.js`)

`watch_file` 17초를 잡아낸 것과 같은 방법이다. 짐작을 늘어놓는 대신 **일어난 자리에서**
기록한다. 정상일 때는 아무것도 적지 않는다.

- 키 누름 → 조합 시작까지 30ms 를 넘으면 그 값을 적는다 (위의 Lexical 분기)
- 조합 한 단계가 그려지기까지 걸린 시간
- **그 글자가 실제로 DOM 에 나타났는가** — 다음 프레임에 확인한다.
  안 나타났으면 그게 사용자가 겪는 바로 그 증상이고, 그때 이렇게 남는다

```
조합  5단계 중 2단계가 화면에 안 나타남 · 총 1840ms · 가장 느린 단계 612ms
```
...
**40배다.** 10만 자가 넘는 문서는 이제 원본 모드로 열린다. 툴바 오른쪽 끝에서 언제든
위지윅으로 바꿀 수 있고, 설정에서 이 동작을 끌 수 있다.

느린 것을 참으라고 하는 대신 **빠른 길을 기본으로** 놓았다.

### 덤 — 기록이 PDF 저장 실패를 잡았다

붙여 둔 기록을 읽다가 오늘 실제로 난 실패를 발견했다.

```
14:30:16 멎음  1556ms · 직전 호출 save_pdf (1.8초 전)
14:32:14 오류  save_pdf — PDF 저장이 시간 안에 끝나지 않았습니다.
14:32:14 느림  save_pdf 120119ms — …OKC_MES_설정가이드.pdf
```

`save_pdf` 는 `recv_timeout` 으로 완료를 기다린다. **그런데 `(async)` 가 빠져 메인
스레드에서 돌고 있었다.** WebView2 의 PrintToPdf 완료 알림은 그 메인 스레드로 와야
하는데 그 스레드가 기다림에 잡혀 있으니 **자기가 기다리는 것을 자기가 막는** 꼴이었다.
120초를 채우고 실패한다.

코드 주석에는 이미 "커맨드 자체는 작업 스레드라 여기서 기다려도 안전하다" 고 적혀
있었다. **주석이 말하는 전제가 코드에 없었던 것이다.** `(async)` 를 붙여 주석이 사실이
되게 했다.
...
| 대상 | 방법 | 결과 |
|---|---|---|
| 조합 중 직렬화를 떼면 얼마나 빨라지나 (293 → 243ms) | 실측(CDP) | 원인이 Lexical 쪽임을 확인 |
| 큰/작은 문서 · 스위치 · 빈 문서의 초기 모드 · 안내 문구 | 단위 6건 | 통과 |
| 작은 문서에서 조합 글자가 보인다 | 헤들리스 브라우저 | 통과 |
| 제목 · 표 셀 복사 · 탭 줄 · 색 (회귀) | 헤들리스 브라우저 28건 | 통과 |

**아직 사람이 볼 것** — 진짜 IME 로 조합이 안 보이는 그 순간. `.mdlog` 의 `조합` 줄이
그때를 잡아 준다. 지금까지는 그 순간의 자료가 하나도 없었다.
