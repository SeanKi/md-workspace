...
:check_build
if not exist "release-out\MD-Notepad-portable.exe" (
  echo ============================================
...
echo [1/2] MD-Notepad portable 복사...
copy /y "release-out\MD-Notepad-portable.exe" "%DEST%\MD-Notepad-portable.exe" >> "%LOG%" 2>&1
if errorlevel 1 (
  echo   [X] MD-Notepad 복사 실패
  pause
  exit /b 1
)
